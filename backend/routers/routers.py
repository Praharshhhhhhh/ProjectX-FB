...
def register_desktop(
    ...
    if not req.public_key and not req.zerotier_node_id:
        raise HTTPException(status_code=400, detail="Must provide either public_key or zerotier_node_id")

    # 1. Idempotency Check: lookup by device_name which is unique per user
    ...
    if peer:
        ...
        if req.zerotier_node_id:
            peer.zerotier_node_id = req.zerotier_node_id
        ...
        peer = DesktopPeer(
            ...
            active=True,
            tunnel_state="connected" if req.public_key else "disconnected"
        )
        db.add(peer)
        db.commit()
        db.refresh(peer)
        
    # 3.5 Notify Gateway immediately (best-effort) for WireGuard
    if req.public_key:
        try:
            import requests
            requests.post(
                f"http://{settings.GATEWAY_HOST}:{settings.GATEWAY_API_PORT}/v1/peers",
                json={"public_key": req.public_key, "allowed_ips": f"{peer.wg_ip}/32"},
...
def disconnect_desktop(
    ...
    try:
        import requests
        requests.delete(
            f"http://{settings.GATEWAY_HOST}:{settings.GATEWAY_API_PORT}/v1/peers/{req.public_key}",
            headers={"X-Gateway-Token": settings.GATEWAY_API_TOKEN},
            timeout=2
        )
    except Exception:
...
def get_desktop_config(
    ...
    return {
        ...
        "zerotier_node_id": peer.zerotier_node_id,
        "zt_network_id": settings.GLOBAL_ZT_NETWORK_ID,
        "active": peer.active,
