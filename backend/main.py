...
async def lifespan(app: FastAPI):
    ...
    if settings.GATEWAY_HOST == "192.168.196.170":
        logger.warning("WARNING: GATEWAY_HOST is using the default value '192.168.196.170'. This might fail in non-default environments.")
    if settings.GATEWAY_API_TOKEN == "change-this-gateway-token":
        logger.warning("WARNING: GATEWAY_API_TOKEN is using the default placeholder token. Please change this in production.")
