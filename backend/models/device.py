    ...
    if req.wg_public_key:
        ...
        if existing:
            if existing.tenant_id != current_user.tenant_id:
                ...
                _t = db.query(Tenant).filter(Tenant.id == current_user.tenant_id).first()
                return {
                    "assigned_ip": existing.wg_ip,
                    "server_pubkey": _t.wg_server_public_key if _t else "",
                    "server_endpoint": _t.wg_server_endpoint if _t else "",
                    "server_endpoint_secondary": _t.wg_server_endpoint_secondary if _t else "",
                    "config": config_str,
...
def _generate_device_config(device: Device, db: Session) -> str:
    ...
    if is_master:
        ...
        # Use the tenant's claimed WG server endpoint (set during claim-wg-server).
        # This is the authoritative public endpoint — do NOT reconstruct from
        # master_device.lan_ip, which is the LAN address and the wrong port.
        tenant_obj = db.query(Tenant).filter(Tenant.id == device.tenant_id).first()
        server_pubkey = tenant_obj.wg_server_public_key if tenant_obj else ""
        server_endpoint = tenant_obj.wg_server_endpoint if tenant_obj and tenant_obj.wg_server_endpoint else "127.0.0.1:51820"
