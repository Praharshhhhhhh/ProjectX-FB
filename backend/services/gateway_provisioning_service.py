    ...
    with SessionLocal() as db:
        ...
        if registry.claimed_state == SubnetProvisioningState.provisioning_zt:
            try:
                ...
                resp = requests.post(
                    "http://192.168.196.170:8080/v1/provision",
                    json={
...
def reconcile_gateway_peers():
    ...
    try:
        resp = requests.post(
            "http://192.168.196.170:8080/v1/peers/reconcile",
...
def prune_stale_desktop_peers():
    ...
    with SessionLocal() as db:
        ...
        for peer in stale_peers:
            ...
            try:
                requests.delete(f"http://192.168.196.170:8080/v1/peers/{peer.public_key}", timeout=2)
            except Exception:
                pass
