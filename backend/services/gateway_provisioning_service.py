...
def reconcile_gateway_peers():
    ...
    try:
        resp = requests.post(
            f"http://{settings.GATEWAY_HOST}:{settings.GATEWAY_API_PORT}/v1/peers/reconcile",
...
def prune_stale_desktop_peers():
    ...
    with SessionLocal() as db:
        ...
        for peer in stale_peers:
            ...
            try:
                requests.delete(f"http://{settings.GATEWAY_HOST}:{settings.GATEWAY_API_PORT}/v1/peers/{peer.public_key}", headers={"X-Gateway-Token": settings.GATEWAY_API_TOKEN}, timeout=2)
            except Exception:
                pass
